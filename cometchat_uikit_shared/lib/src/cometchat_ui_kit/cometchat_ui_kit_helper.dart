...
class CometChatUIKitHelper {
  ...
  static onMessageReplied(BaseMessage message, MessageStatus messageStatus) {
    CometChatMessageEvents.ccMessageReplied(message, messageStatus);
  }
  ...
  static onGroupMemberScopeChanged(
      ui_kit.Action message, User updatedUser, String scopeChangedTo, String scopeChangedFrom, Group group) {
    CometChatGroupEvents.ccGroupMemberScopeChanged(message, updatedUser, scopeChangedTo, scopeChangedFrom, group);
  }

  ///[onGroupMemberBanned] is used to inform the listeners
  ///when the logged-in user bans a group member
  static onGroupMemberBanned(ui_kit.Action message, User bannedUser, User bannedBy, Group bannedFrom) {
    CometChatGroupEvents.ccGroupMemberBanned(message, bannedUser, bannedBy, bannedFrom);
  }

  ///[onGroupMemberKicked] is used to inform the listeners
  ///when the logged-in user kicks a group member from a group
  static onGroupMemberKicked(ui_kit.Action message, User kickedUser, User kickedBy, Group kickedFrom) {
    CometChatGroupEvents.ccGroupMemberKicked(message, kickedUser, kickedBy, kickedFrom);
  }

  ///[onGroupMemberUnbanned] is used to inform the listeners
  ///when the logged-in user unbans a banned user of a group
  static onGroupMemberUnbanned(ui_kit.Action message, User unbannedUser, User unbannedBy, Group unbannedFrom) {
    CometChatGroupEvents.ccGroupMemberUnbanned(message, unbannedUser, unbannedBy, unbannedFrom);
  }
  ...
  static onGroupMemberAdded(
    ...
    CometChatGroupEvents.ccGroupMemberAdded(messages, usersAdded, groupAddedIn, addedBy);
  }
  ...
  static showPanel(Map<String, dynamic>? id, CustomUIPosition uiPosition, WidgetBuilder child) {
  ...
  static ccActiveChatChanged(
      Map<String, dynamic>? id, BaseMessage? lastMessage, User? user, Group? group, int unreadMessageCount) {
