...
class CometChatUIKit {
  ...
  static init(
    ...
    AppSettings appSettings = (AppSettingsBuilder()
          ..subscriptionType = authenticationSettings?.subscriptionType ?? CometChatSubscriptionType.allUsers
          ..region = authenticationSettings?.region
          ..autoEstablishSocketConnection = authenticationSettings?.autoEstablishSocketConnection ?? true
          ..adminHost = authenticationSettings?.adminHost
          ..clientHost = authenticationSettings?.clientHost)
        .build();

    await CometChat.init(authenticationSettings!.appId!, appSettings, onSuccess: (String successMessage) {
      ...
      if (onSuccess != null) {
        try {
          ...
          if (kDebugMode) {
            debugPrint("CometChat SDK is initialized successfully but failed to execute onSuccess callback");
          }
        }
      }
      ...
      if (onError != null) {
        try {
          ...
          if (kDebugMode) {
            debugPrint("CometChat SDK could not be initialized and failed to execute onError callback");
          }
        }
      }
    });
  ...
  static Future<User?> login(String uid,
      {Function(User user)? onSuccess, Function(CometChatException excep)? onError}) async {
    if (!checkAuthSettings(onError)) return null;
    User? loggedInUser = await getLoggedInUser();

    if (loggedInUser == null || loggedInUser.uid != uid) {
      User? user = await CometChat.login(uid, authenticationSettings!.authKey!, onSuccess: (User user) {
        ...
        if (onSuccess != null) {
          try {
            ...
            if (kDebugMode) {
              debugPrint("user login is successful but failed to execute onSuccess callback");
            }
          }
        }
      ...
      if (onSuccess != null) {
        try {
          ...
          if (kDebugMode) {
            debugPrint("user already logged in but failed to execute onSuccess callback");
          }
        }
      }
  ...
  static Future<User?> loginWithAuthToken(String authToken,
    ...
    if (loggedInUser == null) {
      User? user = await CometChat.loginWithAuthToken(authToken, onSuccess: (User user) {
  ...
  static _initiateAfterLogin() {
    ...
    List<ExtensionsDataSource> extensionList = authenticationSettings?.extensions ?? [];

    List<ExtensionsDataSource> aiFeatureList = authenticationSettings?.aiFeature ?? [];
  ...
  static Future<User?> createUser(User user,
      {Function(User user)? onSuccess, Function(CometChatException e)? onError}) async {
    if (!checkAuthSettings(onError)) return null;

    User? resultUser;

    resultUser =
        await CometChat.createUser(user, authenticationSettings!.authKey!, onSuccess: onSuccess, onError: onError);
    return resultUser;
  }
  ...
  static Future<User?> updateUser(User user,
      {Function(User retUser)? onSuccess, Function(CometChatException excep)? onError}) async {
    if (!checkAuthSettings(onError)) return null;

    User? user0;

    user0 = await CometChat.updateUser(user, authenticationSettings!.authKey!, onSuccess: onSuccess, onError: onError);
  ...
  ///
  /// method could throw [PlatformException] with error codes specifying the cause
  static logout({dynamic Function(String)? onSuccess, Function(CometChatException excep)? onError}) async {
    if (!checkAuthSettings(onError)) return;
      ...
      onSuccess: (message) {
        ...
        if (onSuccess != null) {
          try {
            ...
            if (kDebugMode) {
              debugPrint('user logout was successful: $message, but unable to execute custom onSuccess callback');
            }
          }
        }
      },
  ...
  static bool checkAuthSettings(Function(CometChatException e)? onError) {
    if (authenticationSettings == null) {
      if (onError != null) {
        onError(CometChatException("ERR", "Authentication null", "Populate uiKitSettings before initializing"));
      }
      return false;
    }
    ...
    if (authenticationSettings!.appId == null) {
      if (onError != null) {
        onError(CometChatException("appIdErr", "APP ID null", "Populate appId in uiKitSettings before initializing"));
      }
      return false;
    }
    return true;
  ...
  static Future<CustomMessage?> sendCustomMessage(
    ...
    CustomMessage? result = await CometChat.sendCustomMessage(message, onSuccess: (CustomMessage sentMessage) {
      ...
      if (onSuccess != null) {
        try {
          ...
          if (kDebugMode) {
            debugPrint("message sent successfully but failed to execute onSuccess callback");
          }
        }
      }
      ...
      if (onError != null) {
        try {
          ...
          if (kDebugMode) {
            debugPrint("message could not be sent and failed to execute onError callback");
          }
        }
      }
  ...
  static Future<TextMessage?> sendTextMessage(
    ...
    CometChatMessageEvents.ccMessageSent(message, MessageStatus.inProgress);
    TextMessage? result = await CometChat.sendMessage(message, onSuccess: (TextMessage sentMessage) {
      //executing the custom onSuccess handler
      if (onSuccess != null) {
        try {
          onSuccess(sentMessage);
  ...
  static Future<MediaMessage?> sendMediaMessage(MediaMessage message,
    ...
    MediaMessage? result =
        await CometChat.sendMediaMessage(mediaMessage2 ?? message, onSuccess: (MediaMessage sentMessage) {
      //executing the custom onSuccess handler
  ...
  static Future<FormMessage?> sendFormMessage(
    ...
    CometChatMessageEvents.ccMessageSent(message, MessageStatus.inProgress);
    FormMessage? result = await SDKMethods.sendFormMessage(message, onSuccess: (FormMessage sentMessage) {
      //executing the custom onSuccess handler
      if (onSuccess != null) {
        try {
          onSuccess(sentMessage);
  ...
  static Future<CardMessage?> sendCardMessage(
    ...
    CometChatMessageEvents.ccMessageSent(message, MessageStatus.inProgress);
    CardMessage? result = await SDKMethods.sendCardMessage(message, onSuccess: (CardMessage sentMessage) {
      //executing the custom onSuccess handler
      if (onSuccess != null) {
        try {
          onSuccess(sentMessage);
  ...
  static Future<SchedulerMessage?> sendSchedulerMessage(
    ...
    CometChatMessageEvents.ccMessageSent(message, MessageStatus.inProgress);
    SchedulerMessage? result =
        await SDKMethods.sendSchedulerMessage(message, onSuccess: (SchedulerMessage sentMessage) {
      //executing the custom onSuccess handler
      if (onSuccess != null) {
        try {
          onSuccess(sentMessage);
  ...
  static Future<User?> getLoggedInUser(
      {dynamic Function(User)? onSuccess, dynamic Function(CometChatException)? onError}) async {
  ...
  static Future<int> getUnreadNotificationCount() async {
    final cmap = await CometChat.getUnreadMessageCount(
      hideMessagesFromBlockedUsers: true,
    );
    final umap = cmap?['user'];
    if (umap == null || umap.isEmpty) return 0;
    return umap.values.reduce((value, element) => value + element);
  }
  ...
  static Future<BaseMessage?> addReaction(
    ...
    final message = await CometChat.addReaction(messageId, reaction, onSuccess: (reactedMessage) {
      //executing the custom onSuccess handler
      if (onSuccess != null) {
        try {
      ...
      CometChatMessageEvents.ccMessageEdited(reactedMessage, MessageEditStatus.success);
    }, onError: (error) {
      //executing the custom onError handler
      if (onError != null) {
        try {
  ...
  static Future<BaseMessage?> removeReaction(
    ...
    final message = await CometChat.removeReaction(messageId, reaction, onSuccess: (reactedMessage) {
      //executing the custom onSuccess handler
      if (onSuccess != null) {
        try {
          onSuccess(reactedMessage);
