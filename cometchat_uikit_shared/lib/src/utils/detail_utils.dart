...
class DetailUtils {
  static List<CometChatGroupMemberOption> getDefaultGroupMemberOptions(
      {User? loggedInUser, Group? group, GroupMember? member, CometChatTheme? theme}) {
    return [
      ...
      getKickOption(theme: theme),
        ...
        .where((option) => validateGroupMemberOptions(
            ...
            memberScope: member?.uid==group?.owner? GroupMemberScope.owner: member?.scope ?? GroupMemberScope.participant,
            optionId: option.id))
        .toList();
  }
