...
class SwipeTile extends StatefulWidget {
  ...
  const SwipeTile({Key? key, required this.child, required this.menuItems, this.state, this.onTap, this.id, this.theme})
      : super(key: key);
...
class _SwipeTileState extends State<SwipeTile> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final animation = Tween(begin: const Offset(0.0, 0.0), end: Offset(-0.2 * length, 0.0))
    ...
    return GestureDetector(
      ...
      onHorizontalDragEnd: (inputData) {
        ...
        } else if (_controller.value >= 0.5 || inputData.primaryVelocity! < -500) {
...
class SwipeTileOptions extends StatelessWidget {
  ...
  const SwipeTileOptions({Key? key, required this.menuItems, this.id, this.ctheme}) : super(key: key);
        ...
        child: Column(
          ...
          children: [
            ...
            if (item.title != null)
              Text(
                item.title!,
                style: item.titleStyle ?? const TextStyle(color: Colors.white, fontSize: 12),
                maxLines: 1,
    ...
    return PopupMenuButton<CometChatOption>(
      itemBuilder: (context) => menuItems
          .map((item) => PopupMenuItem<CometChatOption>(
                ...
                child: Text(
                  item.title ?? "",
                  style: TextStyle(color: theme.palette.getAccent()).merge(item.titleStyle),
                ),
              ))
          .toList(),
