...
class CometChatReactionList extends StatefulWidget {
  const CometChatReactionList({Key? key,
...
class _CometChatReactionListState extends State<CometChatReactionList> {
  ...
  void initState() {
    ...
    reactionListController = CometChatReactionListController(
        ...
        selectedReaction: widget.selectedReaction ?? ReactionConstants.allReactions
    );
    super.initState();
  }
      ...
      builder: (BuildContext context, ScrollController scrollController) {
        return Container(
          ...
          decoration: BoxDecoration(
              color: widget.reactionListStyle?.background ?? _theme.palette.getBackground(),
              gradient: widget.reactionListStyle?.gradient,
              border: widget.reactionListStyle?.border,
              borderRadius:
               BorderRadius.vertical(top: Radius.circular(widget.reactionListStyle?.borderRadius ?? 16))),
          child: Column(
            children: [
              Padding(
  ...
  Widget _getList(BuildContext context, CometChatTheme theme) {
    return GetBuilder<CometChatReactionListController>(
        ...
        builder: (CometChatReactionListController value) {
          ...
          } else  if (value.isLoading == true && (value.messageReactions.isEmpty)) {
            return _getLoadingIndicator(context, theme);
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: SizedBox(
                        ...
                        child: Row(
                          ...
                          children: [
                            GestureDetector(
                              onTap:()=> value.updateSelectedReaction(ReactionConstants.allReactions),
                              child: Container(

                                padding: const EdgeInsets.symmetric(vertical: 1, horizontal: 5),
                                decoration: BoxDecoration(
                                  color:
                                  value.isSelectedReaction(ReactionConstants.allReactions)? theme.palette.getAccent50():null,
                                  border:
                                  Border.all(
                                      color: theme.palette
                                          .getBackground()
                                ...
                                child: Text("${cc.Translations.of(context).all} ${value.getReactionCount(ReactionConstants.allReactions)}", style: TextStyle(fontSize: theme.typography.name.fontSize, color: theme.palette.getAccent400()).merge(widget.reactionListStyle?.allReactionsTextStyle)),
                              ),
                            ),
                            ...value.messageReactions.keys.map((reaction) => GestureDetector(
                              onTap:()=> value.updateSelectedReaction(reaction),
                              child: Container(
                                padding: const EdgeInsets.symmetric(vertical: 1, horizontal: 5),
                                margin: const EdgeInsets.only(left: 10),
                                decoration: BoxDecoration(
                                  color:
                                  value.isSelectedReaction(reaction)? theme.palette.getAccent50():null,
                                  border:
                                  Border.all(
                                      color: theme.palette
                                          .getBackground()
                                ...
                                child: RichText(
                                  text:  TextSpan(
                                    text: reaction,
                                    style: TextStyle(fontSize: theme.typography.name.fontSize),
                                    children:  <TextSpan>[
                                      TextSpan(text: " ${value.getReactionCount(reaction)}", style: TextStyle(fontSize: theme.typography.name.fontSize, color: theme.palette.getAccent400())),
                                    ],
                                  ),
                                ),
                              ),
  ...
  Widget getReactedUsers(CometChatReactionListController value){
    ...
    return SizedBox(
      height: widget.reactionListStyle?.height ?? MediaQuery.of(context).size.height*.58,
        ...
        itemBuilder: (context, index) {
          if (index >= reactions.length) {
            ...
            return Container(
              ...
              child: widget.loadingIcon ?? Image.asset(
  ...
  Widget getReactionListItem(Reaction reaction, CometChatReactionListController value){
    return CometChatListItem(
      ...
      tailView: Text(reaction.reaction ?? "", style: const TextStyle(fontSize: 34),),
      style: ListItemStyle(
  ...
  Widget? getReactionItemSubtitle(Reaction reaction, CometChatReactionListController value){
    return value.isReactedByMe(reaction.reactedBy!) ? Text(
  ...
  Widget _getNoReactionsIndicator(BuildContext context, CometChatTheme theme) {
    if (widget.emptyStateView != null) {
      ...
      return Container(
        ...
        alignment: Alignment.center,
        child: Text(
          widget.emptyStateText ?? cc.Translations
              .of(context)
              .noReactionsFound,
  ...
  _showErrorDialog(String errorText, BuildContext context,
      CometChatTheme theme, CometChatReactionListController controller) {
