...
class CometChatReactionListController extends GetxController with CometChatMessageEventListener {
  ...
  CometChatReactionListController({required this.messageId,this.reactionsRequestBuilder, this.selectedReaction = ReactionConstants.allReactions});
  ...
  bool canFetchReactions(){
    return hasMoreReactions.containsKey(selectedReaction) && hasMoreReactions[selectedReaction]==true;
  }

  void createReactionsRequest([String reaction=ReactionConstants.allReactions]){
    ReactionsRequestBuilder? requestBuilder= reactionsRequestBuilder ?? ReactionsRequestBuilder();
  ...
  void addReactionInMap(Reaction messageReaction){
    if(messageReactions.containsKey(messageReaction.reaction)){
      ...
      if(messageReactions[messageReaction.reaction!]?.indexWhere((element) => element.reaction==messageReaction.reaction && element.reactedBy?.uid==messageReaction.reactedBy?.uid )==-1) {
  ...
  void onReactionTap(Reaction reaction){
    if(isReactedByMe(reaction.reactedBy!)){
      CometChatUIKit.removeReaction(reaction.messageId!, reaction.reaction!, onSuccess: (reactedMessage) {
  ...
  void removeReactionFromMap(Reaction reaction){
    messageReactions[reaction.reaction!]?.removeWhere((element) => reaction.uid==element.uid && reaction.id==element.id);
    totalReactions--;
    update();
