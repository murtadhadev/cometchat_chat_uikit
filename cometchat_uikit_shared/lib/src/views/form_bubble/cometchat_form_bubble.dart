...
class _CometChatFormBubbleState extends State<CometChatFormBubble> {
  ...
  void initState() {
    ...
    isSentByMe = InteractiveMessageUtils.checkIsSentByMe(widget.loggedInUser, widget.formMessage);
    _populateMap();
  ...
  _checkInteractionGoals() {
    if (widget.formMessage.interactionGoal != null) {
      bool goalAchieved = InteractiveMessageUtils.checkInteractionGoalAchievedFromMap(
  ...
  _populateResponse() {
    for (int i = 0; i < widget.formMessage.formFields.length; i++) {
      if (widget.formMessage.formFields[i] is BaseInputElement) {
        BaseInputElement inputElement = widget.formMessage.formFields[i] as BaseInputElement;

        if (inputElement.response == null && inputElement.defaultValue != null) {
          (widget.formMessage.formFields[i] as BaseInputElement).response =
              (widget.formMessage.formFields[i] as BaseInputElement).defaultValue;
        }
      }
    }
  }
  ...
  Map<String, dynamic>? createResponse() {
    ...
    for (var formField in widget.formMessage.formFields) {
      if (formField.elementType is BaseInputElement) {
        response[formField.elementId] = (formField as BaseInputElement).response;
      }
    }
    return response;
  }
  ...
  bool validateFields() {
    ...
    for (var element in widget.formMessage.formFields) {
      if (element is BaseInputElement) {
        ...
        if (!baseInputElement.optional && !baseInputElement.validateResponse()) {
    ...
    for (var element in widget.formMessage.formFields) {
      if (element is BaseInputElement) {
        if (element is CheckBoxElement) {
          localData[element.elementId] = (element).isChecked?.keys.map((e) => e.toString()).toList();
        } else {
  ...
  Widget getWidget(ElementEntity entity, {bool isSubmit = false}) {
    switch (entity.runtimeType) {
                ...
                children: radioButton.options
                    .map(
                      (option) => RadioListTile(
                        ...
                        activeColor:
                            widget.formBubbleStyle?.radioButtonStyle?.activeColor ?? theme.palette.getPrimary(),
                        value: option.value,
                        ...
                        title: Text(
                          ...
                          style: TextStyle(
                                  ...
                                  fontFamily: theme.typography.subtitle1.fontFamily,
                                  color: theme.palette.getAccent())
                              .merge(widget.formBubbleStyle?.checkBoxStyle?.optionTextStyle),
                        ),
              ...
              Column(
                children: [
                  CometChatSingleSelect(
                    ...
                    selectedOptionBackground: singleSelectStyle.selectedOptionBackground,
                    optionBackground: singleSelectStyle.optionBackground,
                    selectedOptionsTextStyle: singleSelectStyle.selectedOptionTextStyle,
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(singleSelectStyle.borderRadius ?? 6),
                      border: Border.all(
                          width: 2,
                          color: isValidating == true && !singleSelect.optional && !singleSelect.validateResponse()
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Container(
                ...
                width: dropDownElementStyle.width ?? screenWidth(context: context, mulBy: 0.9),
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(6),
                  border: widget.formBubbleStyle?.dropDownStyle?.border ??
                      Border.all(
                          width: 2,
                          color: isValidating == true && !dropDownModel.optional && !dropDownModel.validateResponse()
                              ? theme.palette.getError()
                              : theme.palette.getAccent100()),
                ),
                child: Padding(
                    ...
                    items: dropDownModel.options.map(
                      (val) {
                        return DropdownMenuItem<String>(
                          value: val.value,
                          child: Text(val.label, style: dropDownElementStyle.optionTextStyle),
                        );
                      },
                    ).toList(),
                    onChanged: (String? value) {
                  ...
                  .map(
                    (option) => CheckboxListTile(
                      side: MaterialStateBorderSide.resolveWith(
                        (states) => BorderSide(
                            width: 1.0,
                            color: isValidating == true && !checkBox.optional && !checkBox.validateResponse()
                                ? theme.palette.getError()
                                : theme.palette.getAccent100()),
                      ),
                      contentPadding: EdgeInsets.zero,
          ...
          children: <Widget>[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: isValidating == true && !inputElement.optional && !inputElement.validateResponse()
                      ? theme.palette.getError()
                      : theme.palette.getAccent100(),
                  width: 2,
                ),
          ...
          children: <Widget>[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: const BorderRadius.all(Radius.circular(6.0)),
                border: Border.all(
                  color: isValidating == true && !dateTimeElement.optional && !dateTimeElement.validateResponse()
                      ? theme.palette.getError()
                      : theme.palette.getAccent100(),
                  width: 2,
                ),
      ...
      case ButtonElement:
        ...
        Color? background = isSubmit == true ? submitButtonElementStyle.background : buttonElementStyle.background;
        ...
        return Padding(
          ...
          child: CometChatButtonElement(
            buttonStyle: ButtonElementStyle(
              ...
              buttonTextStyle:
                  isSubmit == true ? submitButtonElementStyle.buttonTextStyle! : buttonElementStyle.buttonTextStyle!,
              borderRadius:
                  isSubmit == true ? submitButtonElementStyle.borderRadius! : buttonElementStyle.borderRadius!,
            ),
