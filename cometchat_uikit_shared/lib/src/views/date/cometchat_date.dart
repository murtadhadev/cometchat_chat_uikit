...
class CometChatDate extends StatelessWidget {
  const CometChatDate({Key? key, this.cdate, this.pattern, this.style = const DateStyle(), this.customDateString})
      : super(key: key);
  ...
  bool _isSameWeek(DateTime dt1, DateTime dt2) {
    return dt1.difference(dt2) <= const Duration(days: 7) &&
        (dt1.add(Duration(days: -dt1.weekday)).day == dt2.add(Duration(days: -dt2.weekday)).day);
  }
  ...
  String _getDateLogic3(DateTime date, BuildContext context,
      {required String timeFormatter, required String dateFormatter2, required String weekFormatter}) {
    DateTime todayDate = DateTime.now();
    if (_isSameDate(todayDate, date)) {
      return DateFormat(timeFormatter).format(date);
    } else if (_isSameDate(todayDate, date.add(const Duration(days: 1)))) {
  ...
  String _getDate(
    ...
    switch (datePattern) {
      ...
      case DateTimePattern.dayDateFormat:
        return _getDateLogic2(date, context, dateFormatter: dateFormatter, weekFormatter: weekFormatter);
      case DateTimePattern.dayDateTimeFormat:
        return _getDateLogic3(date, context,
            weekFormatter: weekFormatter, timeFormatter: timeFormatter, dateFormatter2: dateFormatter2);
      default:
        return _getDateLogic1(date, timeFormatter: timeFormatter);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (customDateString != null) {
      ...
      date = _getDate(pattern, cdate ?? DateTime.now(), context,
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.all(Radius.circular(style.borderRadius ?? 4.0)),
          border: style.border ?? Border.all(width: 1.0, color: const Color(0xff141414).withOpacity(0.14)),
          ...
          gradient: style.isTransparentBackground == true ? null : style.gradient),
      child: Text(
        date,
        style: style.textStyle ?? TextStyle(fontSize: 12.0, color: const Color(0xff141414).withOpacity(0.84)),
      ),
    );
  }
}
