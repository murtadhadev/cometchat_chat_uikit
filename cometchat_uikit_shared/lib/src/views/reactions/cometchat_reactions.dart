...
class CometChatReactions extends StatefulWidget {
    CometChatReactions({Key? key, required this.reactionList, this.theme, this.alignment, this.onReactionTap, this.onReactionLongPress, this.reactionsStyle}):super(key:key);
 ...
 getExtraReactions();
  ...
  void getExtraReactions() {
    ...
    if(reactionList.length>4) {
     bool reactedByMe = reactionList.sublist(3).any((element) => element.reactedByMe==true);
        ...
        extraReactions = GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color:  widget.reactionsStyle?.background ?? Color.alphaBlend((reactedByMe
            ? widget.reactionsStyle?.primaryBackgroundColor ?? _theme.palette.getPrimary().withOpacity(.1):  _theme.palette.getAccent50()),
    _theme.palette.backGroundColor.light) ,
              border:
              Border.all(
                  color: reactedByMe == true
                      ?  _theme.palette.getPrimary().withOpacity(.4) : _theme.palette
                      .getBackground()
                      .withOpacity(0.14),
                  width: 1),
            ...
            child: Text("+ $extraReactionsCount", style: TextStyle(
  ...
  Widget build(BuildContext context) {
    return  Row(
        children: [...reactionList.map((reactionCount) => GestureDetector(
          onTap: () {
          ...
          child: Container(
            ...
            padding:widget.reactionsStyle?.padding ?? const EdgeInsets.symmetric(horizontal: 5, vertical: 2),
            decoration: BoxDecoration(
              color: widget.reactionsStyle?.background ??  Color.alphaBlend((reactionCount.reactedByMe == true
                  ? widget.reactionsStyle?.primaryBackgroundColor ?? _theme.palette.getPrimary().withOpacity(.1):  _theme.palette.getAccent50()),
                _theme.palette.backGroundColor.light) ,
              border:reactionCount.reactedByMe == true
                  ? widget.reactionsStyle?.primaryBorder ?? Border.all(
                  color:_theme.palette.getPrimary().withOpacity(.4),
                  width: 1) : (widget.reactionsStyle?.border ??
                  Border.all(
              ...
              borderRadius:  BorderRadius.all(
                  Radius.circular(widget.reactionsStyle?.borderRadius ?? 22)),
              gradient: widget.reactionsStyle?.gradient
            ),
            child: RichText(
              text:  TextSpan(
                text: reactionCount.reaction,
                style: TextStyle(fontSize: _theme.typography.name.fontSize).merge(widget.reactionsStyle?.reactionTextStyle ),
                children:  <TextSpan>[
                  TextSpan(text: " ${reactionCount.count}", style: TextStyle( fontSize: _theme.typography.name.fontSize, color: _theme.palette.getAccent()).merge(widget.reactionsStyle?.reactionCountTextStyle)),
                ],
              ),
            ),
          ),
